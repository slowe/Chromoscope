...
(function ($) {
	...
	function init(chromo) {
		...
		chromo.bind("load",function(){
			...
			for (var k = 0; k < this.kmls.length; k++) {
				resp = readKML(this,this.kmls[k],30000);
		...
		//console.log("Time to end of KML init:" + (new Date() - chromo.start) + "ms");
	}	
	...
	function readKML(chromo,file){
		//console.log("Time to start of readKML:" + (new Date() - chromo.start) + "ms");
		...
		if(typeof file=="string"){
			...
			if(file.indexOf('://') < 0){
				...
				var duration = 0;
				var overwrite = false;
				for (var i = 1; i < readKML.arguments.length; i++){
					var arg = readKML.arguments[i];
				...
				chromo.message('Loading '+file+'.',true);
				if(file.indexOf(".json") > 0){
					...
					var jqxhr = $.getJSON(fileurl,function(data){
						...
						var total = processFile(_obj,"json",data,overwrite,ngroup);
						_obj.triggerEvent("kml",{total:total,name:docname});
						if(duration > 0) setTimeout(function(file,duration){ readKML(_obj,_obj.file,duration); },duration);
					}).error(function(data) {
					...
					var resp = $.ajax({
						...
						success: function(data) {
							...
							_obj.message('Processing '+docname);
							if(!_obj.container) $('title').text(docname+' | Chromoscope');
							var total = processFile(_obj,"kml",xml,overwrite,ngroup);
							_obj.triggerEvent("kml",{total:total,name:$('Document',xml).children('name').text()});
							//if(callback) callback.call();
							if(duration > 0) setTimeout(function(file,duration){ readKML(_obj,file,duration); },duration);
						},
						error: function(data) {
							_obj.message('Failed to load '+file+'. It may not exist or be inaccessible.',true);
						}
		...
		//console.log("Time to end of readKML:" + (new Date() - chromo.start) + "ms");
	}
	...
	//	overwrite (boolean) = Do we overwrite any previously loaded Placemarkers?
	function processFile(chromo,filetype,data,overwrite,group){

		var overwrite = overwrite ? true : false;
		filetype = (typeof filetype=="string") ? filetype : "kml";
		if(overwrite) chromo.pins = new Array();
		if(!group) group = 0;

		docname = (chromo.pingroups[group].title) ? chromo.pingroups[group].title : "KML";
		if(typeof data != "object") return 0;

		var p = chromo.pins.length;
		var c = chromo;	// Keep a copy of this instance for inside the Placemark loop
		var i = 0;

		// Set the opacity of all the pins (mostly for IE)
		setOpacity($(chromo.container+" .kml"),1.0);

		//console.log("Time until running processFile: " + (new Date() - chromo.start) + "ms");
		var styles = new Array();
		chromo.pinstylecount = 0;
		chromo.pinstyleload = 0;
		var _obj = chromo;
		var added = 0;
		...
		if(filetype == "json"){
			for(i=0 ; i< data.styles.length ; i++){
				...
				chromo.pinstylecount++;
			}
			//console.log("Time to process styles: " + (new Date() - chromo.start) + "ms");
		...
		//console.log("Time to end of processFile: " + (new Date() - chromo.start) + "ms");
		return added;
	}
